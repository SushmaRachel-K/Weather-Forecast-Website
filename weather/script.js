...
function init(resultFromServer) {
    ...
    let weatherDH = document.getElementById('weatherDH');
    ...
    weatherDH.innerText = resultDescription.charAt(0).toUpperCase() + resultDescription.slice(1);
...
function setPositionForWeatherInfo() {
    let weatherC = document.getElementById('weatherC');
    let weatherCHeight = weatherC.clientHeight;
    let weatherCWidth = weatherC.clientWidth;

    weatherC.style.left = `calc(50% - ${weatherCWidth/2}px)`;
    weatherC.style.top = `calc(50% - ${weatherCHeight/1.3}px)`;
    weatherC.style.visibility = 'visible';
}
